...
function escapeLink(unsafe: string): string {
  return unsafe
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
...
function isValidUrl(url: string): boolean {
  ...
  try {
    ...
    const potentialLink = new URL(url);
    potentialLink.toString();
    return true;
  } catch {
    return false;
  }
...
export default function renderPage({
  ...
  const metaTitle = `"${escapeHtml(title)}" | Invite•Info`;
  const metaDescription = `"${escapeHtml(title)}" • Event details: "${escapeHtml(description)}"`;
  const shouldRenderLink = link && isValidUrl(link);
  const shouldRenderEmail = email && emailPattern.test(email);
        ...
        <section>
          <div class="centered">
            ...
            <div class="info" id="where">
              ...
              <p id="location">${escapeHtml(location)}</p>
            </div>
            ${
              shouldRenderLink
              ...
              <p><a href="${escapeLink(link)}" id="link" target="_blank">${escapeHtml(link)}</a></p>
            </div>`
                : ""
            }
            ${
              ...
              <p><a href="mailto:${escapeLink(email)}" id="email" target="_blank">${escapeHtml(email)}</a></p>
            </div>`
                : ""
            }
            <div id="what">
